...
import ChecklistStyles from "../../styles/components/commons/Checklist.style";
  ...
  const renderChecklistItems = data.map((item, index) =>
    isEditMode ? (
      <EditableChecklistItem
        ...
        onDelete={() =>
          onChange([...data.slice(0, index), ...data.slice(index + 1)])
        }
      />
    ) : (
      <ViewChecklistItem
        ...
        onPress={() => {
          ...
          changedItems[index] = {
            ...changedItems[index],
            isComplete: !changedItems[index].isComplete,
          };
          onChange(changedItems);
        }}
      />
      ...
      {isEditMode ? (
        <ChecklistAdderItem
          onPress={(content: string) => {
            if (content && content !== "") {
              onChange([...data, { content, isComplete: false }]);
            }
          }}
        />
      ) : null}
...
export const ViewChecklistItem = ({
  data,
  onPress,
}: ViewChecklistItemProps): JSX.Element => {
  return (
    <View
      onTouchEnd={onPress}
      style={
    ...
    >
      <View style={ChecklistStyles.textInputContainer}>
...
export const EditableChecklistItem = ({
  data,
  onDelete,
  onChangeText,
}: EditableChecklistItemProps): JSX.Element => {
  return (
    <View
      style={
        data.isComplete
    ...
    >
      ...
      <View style={ChecklistStyles.actionButton}>
...
const ChecklistAdderItem = ({ onPress }: ChecklistAdderItemProps) => {
  ...
  return (
    <View style={ChecklistStyles.checklistItemContainer}>
      <View style={ChecklistStyles.textInputContainer}>
        <TextInput
          multiline={true}
