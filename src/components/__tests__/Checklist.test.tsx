...
const checklistProps: ChecklistProps = {
  isEditMode: true,
  data: [],
  onChange: jest.fn((changed) => {
    checklistProps.data = changed;
  }),
};
...
it("creates new checklist item when create button is clicked", () => {
  const { rerender, getByText, getByPlaceholderText, queryByDisplayValue } =
    render(
      <Checklist
        isEditMode={checklistProps.isEditMode}
        onChange={checklistProps.onChange}
  ...
  fireEvent.changeText(
    getByPlaceholderText("New checklist entry"),
    "hello world"
  );
